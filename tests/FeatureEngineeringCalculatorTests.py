...
class TestFeatureEngineeringCalculator(unittest.TestCase):
    ...
    def test_when_period_of_5_days_and_specific_data_is_provided_then_4_nan_is_returned(
            self
    ):
    ...
    def test_when_period_of_5_days_and_specific_data_is_provided_then_sma_values_are_returned(
            self):

        sma = self.calculator.get_simple_moving_average_based_on_close_prices()
        self.assertEqual(sma[4], 152.0,
                         f"Expected 152.0 on day 5, but got {sma[4]}")
        self.assertEqual(sma[5], 151.0,
                         f"Expected 151.0 on day 6, but got {sma[5]}")
        self.assertEqual(sma[6], 150.0,
                         f"Expected 150.0 on day 7, but got {sma[6]}")
        self.assertEqual(sma[7], 149.8,
                         f"Expected 149.8 on day 8, but got {sma[7]}")
        self.assertEqual(sma[8], 149.6,
                         f"Expected 149.6 on day 9, but got {sma[8]}")
        self.assertEqual(len(sma), len(self.data['Close']),
                         f"Expected SMA length of {len(self.data['Close'])}, "
                         f"but got {len(sma)}")
        self.assertFalse(sma[4:].isna().any(),
                         "SMA contains NaN values after day 5, but it should not")
